def parse_line(line: str) -> tuple[tuple[bool, ...], list[set[int]], list[int]]:
    ...
    buttons = [
        set(int(i) for i in x[1:-1].split(","))
        for x in line[right_middle_brace + 2 : left_curly_brace - 1].split()
    ]
    voltage_req = [int(x) for x in line[left_curly_brace + 1 : -1].split(",")]
...
def main():
    ...
    print(
        sum(fewest_to_light(light_req, buttons) for light_req, buttons, _ in machines)
    )
