...
def modify_voltages(
    voltage_reqs: tuple[int, ...], button: frozenset[int], times: int = 1
) -> tuple[int, ...]:
...
def fewest_to_voltages(
    ...
    if any(
        v > 0 and not any(i in b for b in buttons) for i, v in enumerate(voltage_reqs)
    ):
        return 999999999

    button_with_cnts = tuple(
        x for x in ((max_clicks(voltage_reqs, b), b) for b in buttons) if x[0] > 0
    )
